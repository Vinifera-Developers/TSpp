...
public:
    ...
    Int times_b_mod_c(Int const& multiplier, Int const& modulus) const { Int result; Error = xmp_stage_modulus(modulus, PRECISION); Error = XMP_Mod_Mult(result, &reg[0], multiplier, PRECISION); XMP_Mod_Mult_Clear(PRECISION); return result; }
    Int exp_b_mod_c(const Int& e, const Int& m) const { Int result; Error = xmp_exponent_mod(result, &reg[0], e, m, PRECISION); return result; }
    ...
    friend Int Generate_Prime(Straw& rng, int pbits, Int const* dummy = nullptr);
...
private:
    ...
    public:
        RemainderTable(const Int& p) : HasZeroEntry(false)
        {
