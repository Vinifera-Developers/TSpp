...
class Vector2
{
    public:
        ...
        inline Vector2(const float vector[2]) { X = vector[0]; Y = vector[1]; }
    
        inline Vector2 & operator = (const Vector2 & v) { X = v[0]; Y = v[1]; return *this; } 
        ...
        inline Vector2 & operator -= (const Vector2 & v) { X -= v.X; Y -= v.Y; return *this; }  
        inline Vector2 & operator *= (float k) { X = (float)(X*k); Y=(float)(Y*k); return *this; }
        inline Vector2 & operator /= (float k) { k=1.0f/k; X*=k; Y*=k; return *this; }
...
{
    return (float)WWMath::Sqrtf(Length2());
}
